...
_Last updated: 2026-09-18 — version 2.3.0 adds the AI Assistant promo card and is packaged for submission._

## Current status

Version 2.2.0 remains published on the Chrome Web Store (listing checked September 8). Version
2.2.1 (new Functions toolbar fix) was packaged but its publication is superseded by 2.3.0, which
adds the one-time Zoho CRM AI Assistant promo card (ADR 0008) on top of it. 2.3.0 is verified,
live-checked in an authenticated EU org, and packaged as `zoho-crm-devtools.zip`; Chrome Web Store
submission is the next manual step. No product implementation is in flight.
...
1. After 2.3.0 is published, watch the AI Assistant listing's `utm_source=zoho-crm-devtools`
   traffic to judge whether in-extension promotion is worth repeating.
2. Ship the deferred theme-following for the preview: persist the observed Zoho editor theme
...
3. Run a focused live-org regression pass when changing cross-function search or its undocumented
